...
export default function Home() {
  ...
  const [searchResponse, setSearchResponse] = useState<AlgoliaSearchResponse | null>(null);
  ...
  const handleSearchQuery = (e: { target: { value: SetStateAction<string> } }) => {
  ...
  return (
    <div className={"flex flex-col items-center justify-center"}>
      <div className={"flex relative justify-center w-2/3 mb-16"}>
        <input
          className={"flex h-10 rounded-md border px-3 ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 w-full pl-10 py-5 text-md bg-white border-gray-200"}
          type="search"
          autoFocus
      ...
      {searchResponse && searchResponse.nbHits > 0 && (
        <div className="text-gray-500 text-center mb-4">
          Found {searchResponse.nbHits} results in {searchResponse.processingTimeMS}ms
        </div>
      )}

      {/* No results message */}
      {!isLoading && query && searchResponse && searchResponse.nbHits === 0 && !error && (
      ...
      <div className={"grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"}>
        {searchResponse?.hits.map((hit) => (
          <Card 
            ...
            description={hit.summary_20	 || hit.summary_50 || 'No description available'}
            urlx={hit.url}
            id={hit.objectID}
            tags={hit.tags}
          />
